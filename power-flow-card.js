...
class PowerFlowCard extends LitElement {
  ...
  constructor() {
    super();
    // Fully restored Base64 Strings - NO TRUNCATION
    this.svgPaths = {
  ...
  getDynamicBatteryIcon(rawIcon, val) {
    ...
    // Robust detection matching standard mdi battery steps
  ...
  renderDescriptor(type) {
    ...
    if (primaryValue) {
      // Offset 0 keeps it aligned exactly to the left on the line
    ...
    let currentY = -15; // Starting Y coordinate for top string
    
    // We safely map the icon inside SVG using foreignObject with an XHTML namespace
