...
class PowerFlowCard extends LitElement {
  ...
  constructor() {
    super();
    // Restored full base64 strings - DO NOT truncate!
    this.svgPaths = {
      primary:
    ...
    // Shifted lineY1 to -38 to lengthen the white line further off the image
    // Note: Y coordinates for text render logic dynamically below.
  ...
  static get styles() {
    return css`
      ...
      .descriptor-secondary-value {
        ...
        font-size: 26px; /* Slightly larger text per request */
        font-weight: 500;
        opacity: 0.9;
      }
      ...
      .descriptor-value {
        fill: var(--primary-text-color, #ffffff);
        font-size: 32px; /* Slightly larger text per request */
        font-weight: bold;
      }
  ...
  getDynamicBatteryIcon(rawIcon, val) {
    ...
    if (lower.includes("battery") || lower.includes("bess")) {
      ...
      if (num <= 10) return "mdi:battery-10";
      if (num <= 20) return "mdi:battery-20";
      if (num <= 30) return "mdi:battery-30";
      if (num <= 40) return "mdi:battery-40";
      if (num <= 50) return "mdi:battery-50";
      if (num <= 60) return "mdi:battery-60";
      if (num <= 70) return "mdi:battery-70";
      if (num <= 80) return "mdi:battery-80";
      if (num <= 90) return "mdi:battery-90";
      return "mdi:battery";
    }
  ...
  formatValue(stateStr, currentUnit, displayUnitCfg, multiplierCfg, decimalsCfg) {
    ...
    // Check if the input or unit contains the GBP symbol
    ...
    // Position currency symbol as a prefix if it contains £ (e.g., changes £/kWh to £1.50 /kWh)
  ...
  renderDescriptor(type) {
    ...
    if (primaryValue) {
      // Offset 0 keeps it aligned exactly to textX anchor
      rows.push({ type: 'text', text: primaryValue, class: "descriptor-value", offset: 0 });
    }
    
    if (secondaryValue) {
      if (resolvedIcon) {
        // Offset the text so the icon can fit to the left cleanly
        rows.push({ type: 'icon-text', icon: resolvedIcon, text: secondaryValue, class: "descriptor-secondary-value", offset: 30 });
      } else {
    ...
    if (label) {
      rows.push({ type: 'text', text: label, class: "descriptor-label", offset: 0 });
    }

    let currentY = -8; // Slightly shifted up for larger text
    
    // We use <foreignObject> to safely inject standard Home Assistant <ha-icon> tags into the SVG 
    const textNodes = rows.map(row => {
      let node;
      if (row.type === 'icon-text') {
        node = svg`
          <foreignObject x="${anchor.textX}" y="${currentY - 20}" width="28" height="28">
            <div style="display: flex; align-items: center; justify-content: flex-start; width: 100%; height: 100%;">
                <ha-icon icon="${row.icon}" style="color: var(--primary-text-color, #ffffff); --mdc-icon-size: 24px; width: 24px; height: 24px;"></ha-icon>
            </div>
          </foreignObject>
      ...
      currentY += 30; // Drop down 30px spacing between lines to accommodate larger fonts
      return node;
    });
    ...
    return html`
      <ha-card header="${this.config.name || "Power Flow Diagram"}" style="${colorStyle}">
        <div id="svg-overlay">
          ...
          <!-- overflow: visible ensures the extended lines and icons do not get clipped -->
