...
class PowerFlowCard extends LitElement {
 ...
 constructor() {
    ...
    this.svgPaths = {
      ...
      ev: "data:image/svg+xml;base64,",
  ...
  getEntityId(key) {
    ...
    if (this.config.entities && this.config.entities[key]) return this.config.entities[key];
  ...
  formatValue(stateStr, currentUnit, displayUnitCfg, multiplierCfg, decimalsCfg, entityId) {
    ...
    let val = parseFloat(stateStr.replace(/[^\d.-]/g, ''));
    if (isNaN(val)) return `${stateStr} ${currentUnit || ""}`.trim();

    const currencyRegex = /[\$£€¥₹₽₩¢₪₫฿₺]/;
    let symbol = "";
    
    if (currencyRegex.test(stateStr)) symbol = stateStr.match(currencyRegex)[0];
    else if (displayUnitCfg && currencyRegex.test(displayUnitCfg)) symbol = displayUnitCfg.match(currencyRegex)[0];
    else if (currentUnit && currencyRegex.test(currentUnit)) symbol = currentUnit.match(currencyRegex)[0];
    else if (entityId && this._hass && this._hass.states[entityId]) {
      if (this._hass.states[entityId].attributes.device_class === "monetary") {
        symbol = currentUnit || displayUnitCfg || "";
      }
    }
    ...
    if (symbol) {
      return `${symbol}${formattedNum}${displayUnit ? ' ' + displayUnit : ''}`;
    }
  ...
  renderDescriptor(type) {
    ...
    if (primaryEntityId && this._hass && this._hass.states[primaryEntityId]) {
      ...
      primaryValue = this.formatValue(
        state.state, unit, this.config[`${type}_display_unit`], 
        this.config[`${type}_unit_multiplier`], undefined, primaryEntityId
      );
    }
    ...
    if (secondaryEntityId && this._hass && this._hass.states[secondaryEntityId]) {
      ...
      secondaryValue = this.formatValue(
        state.state, unit, this.config[`${type}_secondary_display_unit`], 
        this.config[`${type}_secondary_unit_multiplier`], undefined, secondaryEntityId
      );
    }

    const rows = [];
    if (primaryValue) rows.push({ type: 'text', text: primaryValue, class: "descriptor-value", offset: 0, fontSize: primaryFontSize });
